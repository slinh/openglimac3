...
bool compileShader(GLhandleARB object)
{
  ...
  int status;
  glGetObjectParameterivARB(object, GL_OBJECT_COMPILE_STATUS_ARB, (GLint *)&status);
  if(status==0){
    int length = 0;
    glGetObjectParameterivARB(object, GL_OBJECT_INFO_LOG_LENGTH_ARB, (GLint *)&length);
    if(length>0){
...
GLhandleARB linkShaders(GLhandleARB* object, const unsigned int& nb)
{
  ...
  int status;
  glGetObjectParameterivARB(po, GL_OBJECT_LINK_STATUS_ARB, (GLint *)&status);
  if(status==0){
    int length = 0;
    glGetObjectParameterivARB(po, GL_OBJECT_INFO_LOG_LENGTH_ARB, (GLint *)&length);
    if(length>0){
      GLsizei minlength = std::min(MAX_LOG_STRING,length);
